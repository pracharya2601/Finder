...
exports.signup = (req, res) => {
    ...
    db.doc(`/users/${newUser.handle}`)
        ...
        .then(doc => {
            if(doc.exists){
                return res.status(400).json({handle: 'this username is not available'});
            } else {
...
exports.login = (req, res) => {
    ...
    .catch(err => {
        console.error(err);
        if(err.code === 'auth/wrong-password') return res.status(403).json({general: 'Wrond credentials'});
        if(err.code === 'auth/user-not-found') return res.status(404).json({general: 'No user found'})
        return res.status(500).json({error: err.code});
    });
